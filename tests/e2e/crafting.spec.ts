...
test('keyboard search → units → pins → inventory → reload → complete; backup and reset',async({page})=>{
 ...
 await page.reload();await expect(page.getByLabel('Wood stock',{exact:true})).toHaveValue('3');await expect(page.getByRole('heading',{name:'Arrow · 0 / 11'})).toBeVisible();
 await search.fill('sphere');await page.getByRole('button',{name:'Select Pal Sphere',exact:true}).click();await page.getByRole('button',{name:'Pin craft goal'}).click();await expect(page.getByRole('heading',{name:'Pal Sphere · 0 / 1'})).toBeVisible();
 ...
 await page.getByRole('button',{name:'Complete Arrow',exact:true}).click();await expect(page.getByRole('heading',{name:'Arrow · 11 / 11'})).toBeVisible();await expect(page.getByLabel('Wood stock',{exact:true})).toHaveValue('3');
 ...
 await page.reload();await page.getByRole('link',{name:'Craft',exact:true}).click();await expect(page.getByRole('heading',{name:'Arrow · 11 / 11'})).toBeVisible();await expect(page.getByLabel('Wood stock',{exact:true})).toHaveValue('3');await page.getByRole('button',{name:'Remove Arrow',exact:true}).click();await expect(page.getByRole('heading',{name:'Arrow · 11 / 11'})).toHaveCount(0);
...
test('no results, invalid quantity, unknown imports retained and blocked',async({page})=>{
 ...
 await expect(page.getByRole('button',{name:'Complete future-recipe',exact:true})).toBeDisabled();
 await expect(page.getByLabel('future-stock stock',{exact:true})).toHaveValue('9');
 await page.reload();await expect(unresolved).toBeVisible();
 await expect(page.getByRole('button',{name:'Complete future-recipe',exact:true})).toBeDisabled();
