  ...
  return (
    <div className="app-shell">
      ...
      <main id="main-content" tabIndex={-1}>
        ...
        <p className="catalog-status">{runtime?.selected ? `Catalog: ${runtime.selected.craft.items.filter(i=>i.kind==='craftable').length} craftable items · ${runtime.selected.craft.recipes.length} known recipes · ${runtime.selected.craft.items.filter(i=>i.kind==='raw').length} material boundaries · Game version: ${runtime.selected.manifest.gameVersion??'unverified'}` : catalogError||'Selected catalog unavailable; calculations require exact saved references.'}</p>
