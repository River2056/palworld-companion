...
test('Dexie roundtrip retains unknown records and invalid import never replaces data', async () => {
 ...
 await store.save(data); expect(await store.load()).toEqual(data);
 const text = await store.export(); expect(parseBackup(text)).toEqual(data);
 await expect(store.import('{"version":2}')).rejects.toThrow(); expect(await store.load()).toEqual(data);
 const invalid = JSON.parse(text); invalid.stock.wood = -1;
 await expect(store.import(JSON.stringify(invalid))).rejects.toThrow(); expect(await store.load()).toEqual(data);
 await store.reset(); expect(await store.load()).toEqual(emptyWorkspace());
 await store.import(text); expect(await store.load()).toEqual(data); await store.delete();
});
