...
export function Craft({data,update}:WorkspaceProps) {
 ...
 const index=useMemo(()=>new Fuse(snapshot?.craft.items.map(i=>({...i,aliases:[...(i.aliases??[]),i.id.replaceAll('-',' '),i.name+'s']}))??[],{keys:['name','id','aliases'],threshold:0.35,ignoreLocation:true}),[snapshot]);
 ...
 const matches=query.trim()?index.search(query.trim()).map(r=>r.item):[...snapshot.craft.items.filter(i=>i.kind==='craftable')].sort((a,b)=>{const rank=(id:string)=>data.recent.includes(id)?data.recent.indexOf(id):data.recent.length;return rank(a.id)-rank(b.id);}).slice(0,50);
 const selectedItem=snapshot.craft.items.find(i=>i.id===item);
 ...
 return <div className="stack"><section className="card"><h2>Find your next craft</h2><p>Selected reference: {snapshot.manifest.datasetId} · {snapshot.id} · {snapshot.manifest.verificationStatus}</p><label>Search items and recipes<input value={query} onChange={e=>{setQuery(e.target.value);setItem('');setDuplicate(false);}} onKeyDown={e=>{if(e.key==='Escape')setItem('');if(e.key==='ArrowDown'){e.preventDefault();document.querySelector<HTMLButtonElement>('.search-results button')?.focus();}}}/></label><p>Search craftable items and raw materials by typo, partial name or alias. Arrow down to results; Enter to select. Empty search shows recent crafts and the first 50 craftable entries; typed searches cover the complete catalog.</p>{!item&&<div className="search-results">{matches.map(i=><button key={i.id} aria-label={`Select ${i.name}`} onClick={()=>{setItem(i.id);setRecipeId(snapshot.craft.defaultRecipeByItem[i.id]??'');setOverrides({});}}>{i.name}</button>)}{!matches.length&&<p>No items found.</p>}</div>}
 {item&&selectedItem?.kind==='raw'?<div className="recipe"><h3>How to obtain {selectedItem.name}</h3>{selectedItem.acquisition?.length?<ul>{selectedItem.acquisition.map(method=><li key={method}>{method}</li>)}</ul>:<p>No acquisition guidance is available in this catalog.</p>}<a href={selectedItem.source.revision_url??selectedItem.source.url}>Material source · {selectedItem.source.attribution}</a><p>{selectedItem.source.license}</p></div>:item&&<div className="recipe"><h3>{name(item)}</h3>{alternatives.length>1?<label>Root recipe<select aria-label="Root recipe" value={recipeId} onChange={e=>setRecipeId(e.target.value)}>{alternatives.map(r=><option key={r.id} value={r.id}>{r.variant??r.id} · yield {r.output_count}</option>)}</select></label>:<p>One known recipe available.</p>}
 {selected.status==='resolved'&&<><p>Recipe: {selected.recipe.id}{selected.recipe.variant?` · ${selected.recipe.variant}`:''} · Stations: {selected.recipe.stations.join(', ')||'Unknown'} · Unlock: {selected.recipe.unlock_level??'Unknown'}</p><p>Output per run: {selected.recipe.output_count} finished units.</p><a href={selected.recipe.source.revision_url??selected.recipe.source.url}>Recipe source · {selected.recipe.source.attribution}</a><p>{selected.recipe.source.license}</p></>}
