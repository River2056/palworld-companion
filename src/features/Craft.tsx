...
import { plan, type Goal } from '../domain/planner';
...
export function Craft({data,update}:WorkspaceProps) {
 ...
 const [duplicate,setDuplicate]=useState(false); const [pinError,setPinError]=useState('');
 async function pin(existing?:Goal) {
  if (!selected || !preview) return;
  try {
   const goals=existing?data.goals.map(g=>g.id===existing.id?{...g,quantity:g.quantity+count}:g):[...data.goals,{id:crypto.randomUUID(),item:selected.id,quantity:count,completed:0,notes:''}];
   plan(catalog,goals,data.stock);
   if(await update({...data,goals,recent:[selected.id,...data.recent.filter(id=>id!==selected.id)].slice(0,10)})) {setDuplicate(false);setPinError('');}
  } catch {setPinError('Cannot pin: goal quantity or calculated amounts exceed safe whole-number limits.');}
 }
 ...
 return <div className="stack"><section className="card"><h2>Find your next craft</h2><label>Search recipes<input value={query} onChange={e=>{setQuery(e.target.value);setSelected(undefined);setDismissed(false);setDuplicate(false);setPinError('');}} onKeyDown={e=>{if(e.key==='Escape'){setDismissed(true);setSelected(undefined);} if(e.key==='ArrowDown'){e.preventDefault();document.querySelector<HTMLButtonElement>('.search-results button')?.focus();}}} aria-describedby="search-help" /></label><p id="search-help">Typo, partial name or alias. Arrow down to results, Tab to move, Enter to select, Escape to dismiss. Empty search shows recent selections first.</p>
 ...
 {selected&&<div className="recipe"><h3>{selected.name}{selected.variant?` (${selected.variant})`:''}</h3><p>Stations: {selected.stations.join(', ')||'Unknown'} (selected supported stations, not exhaustive). Unlock: {selected.unlock_level??'Unknown'}.</p><p>Output per run: {selected.output_count} finished units.</p><p>One reference recipe available; alternate recipes are not included.</p><p><a href={selected.source.revision_url??selected.source.url} target="_blank" rel="noreferrer">Recipe source · {selected.source.attribution}</a> · {selected.source.license}</p>{selected.notes&&<p>{selected.notes}</p>}<label>Desired finished units<input type="number" min="1" step="1" value={quantity} onChange={e=>setQuantity(e.target.value)}/></label>{error?<p role="alert">{error}</p>:preview&&<><p>{preview.goals[0].batches} batches · {preview.goals[0].output} output · {preview.goals[0].surplus} surplus</p><h4>Direct ingredients</h4><ul>{preview.direct.map(r=><li key={r.item}>{itemName(r.item)} × {r.required}</li>)}</ul><IngredientTree catalog={catalog} item={selected.id} quantity={count}/></>}<button disabled={!preview} onClick={()=>{if(data.goals.some(g=>g.item===selected.id))setDuplicate(true);else void pin();}}>Pin craft goal</button>{duplicate&&<section aria-label="Duplicate pin choice"><p>This recipe is already pinned. Add {quantity} finished units to an existing goal (fulfilled progress stays unchanged), or keep a separate goal.</p>{data.goals.filter(g=>g.item===selected.id).map(g=><button key={g.id} disabled={!preview||!Number.isSafeInteger(g.quantity+count)} onClick={()=>void pin(g)}>Increase existing goal · {g.completed}/{g.quantity} · {g.notes||g.id}</button>)}<button disabled={!preview} onClick={()=>void pin()}>Create separate pin</button><button onClick={()=>setDuplicate(false)}>Cancel pin</button></section>}{pinError&&<p role="alert">{pinError}</p>}</div>}
