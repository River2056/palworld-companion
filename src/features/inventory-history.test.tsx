...
test('failed completion keeps the active goal and partial progress can explicitly finish it', async () => {
  ...
  const initial: Workspace = {...emptyWorkspace(), goals: [{id: 'g', item: 'arrow', quantity: 11, completed: 4, notes: 'Partial'}], stock: {wood: 0}};
  try {
    ...
    await user.click(screen.getByRole('button', {name: 'Complete Arrow'}));
...
test('priority changes skip retained history; unknown completed recipes can reopen without data loss', async () => {
  ...
  const first = {id: 'a', item: 'arrow', quantity: 1, completed: 0, notes: 'First'};
  const unknown = {id: 'h', item: 'future-recipe', quantity: 2, completed: 2, notes: 'Retain unknown'};
  const last = {id: 'b', item: 'cloth', quantity: 1, completed: 0, notes: 'Last'};
  const initial: Workspace = {...emptyWorkspace(), goals: [first, unknown, last]};
  try {
    await store.save(initial);
    render(<WorkspaceHarness initial={initial} store={store}/>);
    expect(screen.getByRole('button', {name: 'Move Arrow up'})).toBeDisabled();
    ...
    await user.click(screen.getByRole('button', {name: 'Reopen Unknown: future-recipe (progress only)'}));
    await waitFor(() => expect(screen.getByText('Blocked: unknown recipe retained.')).toBeVisible());
...
test('saving an unchanged zero count refreshes only that entry; invalid counts never write', async () => {
  ...
  const form = screen.getByLabelText('Wood stock').closest('form')!;
...
test('complete -> history -> reload -> reopen preserves the goal and stock while recomputing allocations', async () => {
  ...
  const original = {id: 'first', item: 'arrow', quantity: 11, completed: 0, notes: 'Keep this goal'};
  const second = {id: 'second', item: 'arrow', quantity: 11, completed: 0, notes: 'Next goal'};
  ...
  try {
    ...
    await user.click(screen.getAllByRole('button', {name: 'Complete Arrow'})[0]);
    ...
    const history = screen.getByRole('region', {name: 'Completed goal history'});
    ...
    expect(screen.getAllByRole('button', {name: 'Complete Arrow'})).toHaveLength(2);
  } finally { await store.delete(); }
});
...
test('manual save timestamps only the edited entry; failed save keeps old count and timestamp for retry', async () => {
  ...
  try {
    await store.save(initial);
    render(<WorkspaceHarness initial={await store.load()} store={store}/>);
    expect(screen.getByText(/Stale manual counts can misstate shortages/)).toBeVisible();
