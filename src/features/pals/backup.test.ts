...
it('exports and imports all three stores with unknown IDs and missing original parents preserved',async()=>{
 ...
 await replacePalBackup(s,preview.backup);expect(await s.snapshot()).toMatchObject(snapshot);
 expect(createPalBackup(await s.snapshot(),new Date(backup.exportedAt)).snapshot).toEqual(backup.snapshot);
 expect((await s.snapshot()).routes[0].catalogBinding).toEqual({state:'legacy-unbound',claimedVersion:'legacy'});
});
it('rejects malformed, duplicate and broken cross-reference imports before any writes',async()=>{
 ...
 for(const mutate of mutations){const backup=createPalBackup(snapshot);mutate(backup.snapshot);await expect(replacePalBackup(s,backup)).rejects.toThrow();expect(await s.snapshot()).toMatchObject(snapshot);}
...
it('rolls back clears and puts when a later table write fails',async()=>{
 ...
 await expect(replacePalBackup(s,changed)).rejects.toThrow('disk failure');expect(await s.snapshot()).toMatchObject(snapshot);
});
...
it('previews and cancels without writes, and requires explicit confirmation',async()=>{
 ...
 expect(screen.getByRole('button',{name:'Replace Pal data'})).toBeDisabled();expect(await s.snapshot()).toMatchObject({pals:[],bases:[],routes:[]});
 fireEvent.click(screen.getByRole('button',{name:'Cancel Pal import'}));expect(screen.queryByText('Import preview — no data changed')).not.toBeInTheDocument();expect(await s.snapshot()).toMatchObject({pals:[],bases:[],routes:[]});
});
it('keeps preview on restore failure and supports retry',async()=>{
 ...
 expect(await screen.findByRole('alert')).toHaveTextContent('disk failure');expect(screen.getByText('Import preview — no data changed')).toBeInTheDocument();expect(await s.snapshot()).toMatchObject({pals:[],bases:[],routes:[]});
 fireEvent.click(screen.getByRole('button',{name:'Replace Pal data'}));await waitFor(()=>expect(screen.getByRole('status')).toHaveTextContent('restored'));expect(await s.snapshot()).toMatchObject(snapshot);
});
