...
export function CatalogMigrationPanel() {
 ...
 async function show(p:MigrationPreview,c:CatalogSnapshot){
  ...
  const personal=await workspaceStore.transaction('r',workspaceStore.pals,workspaceStore.routes,workspaceStore.bases,async()=>({pals:await workspaceStore.pals.toArray(),routes:await workspaceStore.routes.toArray(),bases:await workspaceStore.bases.toArray()}));
 ...
 async function prepare(){try{const c=candidate??await createBundledCatalogSnapshot();setCandidate(c);const data=await workspaceStore.load();const goals=Object.fromEntries(data.goals.map(g=>[g.id,decisions[g.id]??(g.catalogBinding?.state!=='bound'?(ack?'migrate':'keep'):'migrate')]));await show(await previewCatalogMigration(workspaceStore,c,{goals,routes:routeDecisions,acknowledgeLegacy:ack}),c);}catch(e){setError(String(e));}}
